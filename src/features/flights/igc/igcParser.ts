...
export const parseIgcFile = (
    ...
    const track = extractBRecords(lines, header.flightDate);
    const lxData = extractLXNAVSpecifics(lines);

    // Calculate metrics directly from the IGC content to avoid drift between different parsers.
    ...
    const data: FlightRecord = {
        ...
        // Header metadata (store null instead of placeholder strings if the value is missing)
        ...
        flightDate: header.flightDate ?? null,
        ...
        // Timing (from extracted track timestamps)
        takeoffTime: track[0]?.timestamp ?? null,
        landingTime: track[track.length - 1]?.timestamp ?? null,

        // Metrics (aligned with your new DB columns / DTO)
    ...
    data.durationSeconds = data.takeoffTime && data.landingTime
        ? Math.max(0, Math.floor((Date.parse(data.landingTime) - Date.parse(data.takeoffTime)) / 1000))
        : null;
...
const extractHeader = (lines: string[]) => {
    ...
    if (!dateLine) {
        throw new Error("IGC missing HFDTE flight date")
    }
    const d = dateLine.substring(10, 16);
    const flightDate = `20${d.substring(4, 6)}-${d.substring(2, 4)}-${d.substring(0, 2)}`;
...
const extractBRecords = (lines: string[]): GPSPoint[] => {
    ...
    return lines
        ...
        .map(line => {
            ...
            const tAbs = parseTimeSec(rawTime);
            ...
            return {
                ...
                alt: parseInt(line.substring(30, 35), 10),
            };
        });
};
...
/**
 * Umrechnung der IGC Koordinaten (DDMMmmmN) in Dezimalgrad
 */
...
const extractLXNAVSpecifics = (lines: string[]) => ({
    jwt: lines.find(l => l.includes('LXCTDEVICE'))?.split(':').pop()?.trim() || null,
    activityId: lines.find(l => l.includes('LXCTACTIVITY'))?.split(':').pop()?.trim() || null,
    isVerified: lines.some(l => l.startsWith('G'))
});

/**
 * Berechnet Statistiken basierend auf dem Track
 */
export const calculateFlightStats = (track: GPSPoint[]) => {
    if (track.length < 2) return { durationSeconds: 0, maxAltitude: 0, distanceKm: 0 };

    const duration = (new Date(track[track.length - 1].timestamp).getTime() - new Date(track[0].timestamp).getTime()) / 1000;
    ...
    return { durationSeconds: Math.floor(duration), maxAltitude: maxAlt, distanceKm: Math.round(dist * 100) / 100 };
};
...
const sha256Sync = (message: string): string => {
    ...
    const hash: number[] = (sha256Sync as any).h = (sha256Sync as any).h || [];
    const k: number[] = (sha256Sync as any).k = (sha256Sync as any).k || [];
    ...
    // Fix: Explizite Typisierung und Initialisierung mit 0
    const words_copy: number[] = [];
    for (i = 0; i < messageLength; i++) {
        const index = i >> 2;
        words_copy[index] = (words_copy[index] || 0) | (messageOctets.charCodeAt(i) << (24 - (i % 4) * 8));
    }

    const bitLength = messageLength * 8;
    const lastIndex = messageLength >> 2;
    words_copy[lastIndex] = (words_copy[lastIndex] || 0) | (0x80 << (24 - (messageLength % 4) * 8));

    const finalLengthIndex = (((messageLength + 8) >> 6) << 4) + 15;
    ...
    for (i = 0; i < words_copy.length; i += 16) {
        ...
        // Sicherstellen, dass w immer 64 Einträge für die Iteration hat
        ...
        for (j = 0; j < 64; j++) {
            ...
            const t1 =
                (hashState[7] +
                    (((hashState[4] >>> 6) | (hashState[4] << 26)) ^
                        ((hashState[4] >>> 11) | (hashState[4] << 21)) ^
                        ((hashState[4] >>> 25) | (hashState[4] << 7))) +
                    ((hashState[4] & hashState[5]) ^ (~hashState[4] & hashState[6])) +
                    k[j] +
                    w[j]) |
                0;
            const t2 =
                ((((hashState[0] >>> 2) | (hashState[0] << 30)) ^
                    ((hashState[0] >>> 13) | (hashState[0] << 19)) ^
                    ((hashState[0] >>> 22) | (hashState[0] << 10))) +
        ...
        for (j = 0; j < 8; j++) {
            hashState[j] = (hashState[j] + oldHash[j]) | 0;
        }
    }
